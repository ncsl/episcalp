...
def plot_2d_ss_space(
    ...
    (
        ...
        p_row_ranks,
        p_col_ranks,
    ) = _identifySS_plotRanks(A_mean, thresh_percSS)
    ...
    ch_influential_ind, ch_influenced_ind = _identify_influence_channels(
        A_mean, thresh_percConn
    )
...
if __name__ == "__main__":
    ...
    deriv_root = Path(
        "D:/OneDriveParent/OneDrive - Johns Hopkins/Shared Documents/bids/derivatives"
    )
            ...
            for task in tasks:
                ...
                for run in runs:
                    ...
                    if sf > 256:
                        resample_sfreq = 256
                    raw = read_scalp_eeg(
                        bids_path, reference=None, resample_sfreq=resample_sfreq
                    )
                    X = raw.get_data()
                    ...
                    savename = (
                        ...
                        / "figures"
                        / "sourcesink"
                        / bids_path.basename.replace(".edf", "_2dplot.png")
                    )
                    plot_2d_ss_space(
                        A_mats, ch_names, sf, deriv_path=deriv_root, savename=savename
                    )
